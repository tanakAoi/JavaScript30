...
const livedShortToLong = inventors.sort((a, b) => {
...
console.table(livedShortToLong)

// 5.5 Give us an array of the inventors lifetime with full names
const lifetimes = inventors.map(inventor => {

    const lifetime = inventor.passed - inventor.year;
    const fullName = `${inventor.first} ${inventor.last}`;

    return `${fullName} : ${lifetime}`;

});

console.table(lifetimes)
...
const boulevardsWithDe = links
    ...
    .filter(boulevardsName => boulevardsName.includes('de'));
...
// 8. Sum up the instances of each of transportation
const sumUpTransportation = transportation.reduce((obj, item) => {
...
console.log(sumUpTransportation)